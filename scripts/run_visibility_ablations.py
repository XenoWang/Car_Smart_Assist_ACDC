...
N_PER_SPLIT = 60          # train / calib / test 各抽多少张
...
def evaluate_run(
    ...
    """算对比所需的指标。口径与 evaluate_visibility.py 保持一致。"""

    def pick(idx):
        return rng.choice(idx, size=min(N_PER_SPLIT, len(idx)), replace=False)

    tr_i, ca_i, te_i = pick(tr_idx), pick(ca_idx), pick(te_idx)
    tr = scorer.score_arrays([refs[i] for i in tr_i])
    ca = scorer.score_arrays([refs[i] for i in ca_i])
    te = scorer.score_arrays([refs[i] for i in te_i])

    tr_m = float(np.mean([s.recon_mean for s in tr]))
    ca_m = float(np.mean([s.recon_mean for s in ca]))
    te_m = float(np.mean([s.recon_mean for s in te]))

    te_verdicts = gate.judge_many(te)
    fpr = sum(1 for v in te_verdicts if v.level is VisibilityLevel.BLIND) / max(len(te), 1)

    # 合成退化：召回率 + 信息量
    base = [refs[i] for i in pick(te_idx)]
    synth: dict[str, Any] = {}
    for kind in KINDS:
        curve = []
    ...
    return {
        ...
        "recon_calib": ca_m,
        "recon_test": te_m,
        "overfit_ratio": ca_m / tr_m if tr_m > 0 else float("nan"),
        "test_ratio": te_m / tr_m if tr_m > 0 else float("nan"),
        "test_fpr": fpr,
        "synth": synth,
...
def main() -> int:
    ...
    logger.info(
        "统一划分（所有预设共用）：训练 %d / 校准 %d / 测试 %d",
        len(tr_idx), len(ca_idx), len(te_idx),
    )
    ...
    for i, preset in enumerate(presets, 1):
        ...
        scorer = VisibilityScorer.from_checkpoint(ckpt, device=device, cfg=cfg.get("model", {}))
        gate = VisibilityGate.from_config(cfg)

        rng = np.random.default_rng(0)
        metrics = evaluate_run(scorer, gate, refs, ref_paths, tr_idx, ca_idx, te_idx, rng)
        ...
        logger.info(
            "  %s 完成: 参数 %.2fM 过拟合比 %.3f 测试误报 %.1f%% 平均召回 %.1f%% (%.0f 秒)",
            tag, metrics["params_m"], metrics["overfit_ratio"],
            metrics["test_fpr"] * 100, metrics["mean_recall"] * 100, train_s,
        )
...
def write_report(results: dict[str, Any], out_dir: Path) -> int:
    ...
    L: list[str] = [
        ...
        "## 总表",
        "",
        "| 预设 | 参数(M) | 过拟合比 | 测试误报 | 平均召回 | " + " | ".join(KINDS) + " | 训练(s) |",
        "|------|--------|---------|---------|---------|" + "---|" * len(KINDS) + "--------|",
    ]
    for tag in order:
        m = results[tag]
        ...
        L.append(
            ...
            f"{m['test_fpr']:.1%} | {m['mean_recall']:.1%} | {rec} | {m['train_seconds']:.0f} |"
        )
    ...
    fps = {round(results[t]["test_fpr"], 6) for t in order}
    ...
    if degenerate:
        L += [
            "## ⚠️ 本次消融无效（结果逐位相同）",
            "",
            f"全部 {len(order)} 组的平均召回与测试误报**完全相同**"
    ...
    print("消融实验完成")
    print("=" * 78)
    print(f"  {'预设':<20}{'参数M':>8}{'过拟合比':>10}{'误报':>8}{'平均召回':>10}")
    print("  " + "-" * 74)
    for tag in order:
        m = results[tag]
        print(
            f"  {tag:<20}{m['params_m']:>8.2f}{m['overfit_ratio']:>10.3f}"
            f"{m['test_fpr'] * 100:>7.1f}%{m['mean_recall'] * 100:>9.1f}%"
        )
