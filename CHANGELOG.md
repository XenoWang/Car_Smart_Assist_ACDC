...
- 天气类别用于描述当前状况和风险线索，类别不确定只记录为诊断，不单独请求接管；能见度门控判为 BLIND（清晰度极差）时请求驾驶员接管。
- 将 ACDC 官方 train+val 开发数据按视频序列整组切分训练/验证（默认 80/20），不让同视频跨集合；官方 test 保持留出。缩放图像一次缓存；用验证损失选 best、早停，并保存带数据签名的 last 检查点用于续训。
- 增加 `configs/model/weather_classifier.yaml` 集中管理模型、视觉阈值、训练和设备参数；`device=auto` 优先可用 CUDA。
- 推理管线检测到天气权重后自动加载，单独记录天气结果与耗时；只有天气识别、缺少目标检测时仍请求接管。没有训练权重时保留原有失效安全输出。
- 当前天气分类模型使用 ACDC 官方 train+val 按序列重切分训练/验证，官方 test 未参与选优；仍需独立评估误报/漏报。视觉代理比例不代表实际积水深度、路面积雪覆盖率或雾中可视距离。
- 验证：`.venv` 全量测试 339 项通过；天气训练、检查点续训和管线相关 9 项测试通过，涉及 Python 文件的 Ruff 检查通过。

### 天气不确定性不单独触发接管（2026-09-26，未发布）

- 按用户策略修正：天气模型类别缺失或置信度不足只记录到风险诊断，不再单独导致 `unable_to_judge` 或接管请求；其他必需感知结果的缺失仍按接管规则处理。
- 明确天气类别本身不代表天气严重程度；接管由能见度清晰度门控及其他必要感知/风险规则决定，BLIND 时直接请求接管。
- 验证：`.venv` 中天气模型、风险策略与推理管线相关测试 157 项通过，修改范围 Ruff 检查通过。

### 图像识别风险与接管规则（2026-09-25，未发布）

- 实装 `advisory/policy/risk.py`：按本帧天气、能见度、全部目标的方向/距离/置信度和距离不确定度分级，记录原因、触发规则与主要风险目标。
- 实装 `handover_rules.py`：必需感知信息不足或异常、明确 critical 风险、可信接管头要求均请求接管；低等级头输出不能抵消其他接管条件。
- 增加 `RiskLevel.UNKNOWN` 和 `object_detection_available`，区分未知风险与已知风险、检测未运行与成功检测但未检出目标。
- `AdvisoryGenerator.from_perception()` 接入真实规则与固定提示，新增 `policy_details`；只有能见度而没有感知结果时明确请求接管。
- 策略可调项集中在 `configs/model/advisory_llm.yaml → advisory.policy`，由 `config/policy.py` 校验、冻结并供生成器首次调用时加载；保留既有阈值与全局环境设置。
- 配置错误进入未知风险接管兜底；保留原 BLIND 门控直达接管路径。同步 `TECH_STACK.md`、接管策略文档与 ADR 0002。
- 本次实现的是识别结果上的规则决策与接管请求；Stage 1 检测/测距、LLM 和实际车辆控制仍待实现，阈值尚需真实场景标定。
- 验证：项目 `.venv` 中全量测试 330 项通过（本轮增加 110 项）；修改范围 Ruff 检查通过，沿用原项目对现有枚举迁移提示 UP042 的豁免。

### 重复代码精简（2026-09-25，未发布）

- 图像读取和 BILINEAR 缩放合并为 `dataset.read_rgb_image`，供缓存、实时数据集、评分、评估、消融和推理入口复用；保留各调用方原有异常处理策略。
- 校准均值、标准差和分位数统一由 `CalibrationStats.from_errors` 计算，一次求出 p95/p99；拒绝空数据、NaN 和无穷大。
- 自编码器 `forward` 复用 `encode`，消除重复的编码步骤；模型结构与权重键名保持兼容。
- 前车/来车筛选共用方向查询，改为惰性迭代，省去临时候选列表；演示脚本移除无用的并行遍历。
- 新增灰度/RGB/RGBA 图像读取等价性、文件关闭和校准边界回归检查。
- 验证：项目 `.venv` 中完整测试 220 项通过（含 GPU 最小训练/续训），修改文件静态检查通过。

### 配置集中与 GPU 自动选择（2026-09-24，未发布）

- 将模型结构、特征打分、门控阈值、训练与增强的兼容默认值集中到 `src/car_smart_assist/config/visibility.py`；
  保留原有数值，运行配置继续从 `configs/model/visibility.yaml` 覆盖，模型相关模块不再分别维护这些默认值。
- 修复 `train.augment=false` 未生效；新增 `train.augmentation.contrast_range` / `brightness_range`，支持范围校验。
- `device=auto` 自动检测当前 PyTorch CUDA 可用性，优先 GPU，无 CUDA 时记录原因并回退 CPU；
  显式 CUDA 不可用或设备编号越界时清楚报错，启动日志显示实际 GPU 名称和 CUDA runtime。
- 不支持 bf16 的 CUDA GPU 回退到 GPU float32；fp16 使用 GradScaler，并保存/恢复缩放器状态。
  不支持的 dtype 和 optimizer 不再被静默忽略。
- `.venv` 中测试 209 项通过，包含本机 RTX 5070 / CUDA 12.9 的 bf16、fp16 最小训练和 fp16 续训。

### 运行效率与异常处理优化（2026-09-24，未发布）

- 重建误差改为批量池化与统计；校准只计算逐图均方误差，省去分块分位数计算。
- 图像批量转张量一次完成堆叠与设备传输；高频掩膜采用有容量上限的缓存，省去逐帧坐标网格和频谱移位。
- 数据集直接生成连续 CHW float32 张量，光度增强在独立副本上原地执行，减少中间数组与内存拷贝。
- 修复数据集预览后 pickle 内联整份 memmap 的问题：序列化时排除已打开的缓存，由 worker 重新按路径打开。
- 训练和验证损失在设备上累计，每轮仅取回一次标量；保持按样本加权，拒绝空训练/验证加载器。
- 续训先在 CPU 读取检查点，减少临时显存占用；训练结束只读取一次 best 检查点并复用其内容。
- 推理输入尺寸跟随检查点；未显式传入 scoring 时保留检查点中的打分配置。
- 门控报错或返回数量不足时跳过后续感知并请求接管，保留原始异常原因；空批次直接返回。
- 新增回归覆盖：预览后的缓存序列化、图像拷贝及增强等价性、尾批次损失加权、配置回退和最小训练保存/校准流程。
- 验证：项目 `.venv` 中运行 `pytest tests`，200 项通过；本次涉及的 Python 文件通过 Ruff 检查。尚未测量真实训练的 GPU 吞吐和峰值显存。
