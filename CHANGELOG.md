...
- 按 ACDC 官方 fog/night/rain/snow 的 train/val 子集训练；按视频序列检查泄漏，缩放后的图像一次缓存；用验证损失选 best、早停，并保存带数据签名的 last 检查点用于续训。
...
- 当前本地无 ACDC 图片，未运行真实数据训练或误报/漏报评估；视觉代理比例不代表实际积水深度、路面积雪覆盖率或雾中可视距离。
